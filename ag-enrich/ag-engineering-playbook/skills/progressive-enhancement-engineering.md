---
id: progressive-enhancement-engineering
name: Progressive Enhancement Engineering
domain: ag-engineering-playbook
version: 1.0.0
---
# Progressive Enhancement Engineering
**Purpose:** Apply progressive enhancement engineering with the rigor and specificity that profitable agency delivery demands — where client expectations, fixed budgets, and quality standards leave no room for guesswork.
...
For progressive enhancement engineering specifically, the gap between doing this well and doing it poorly is the gap between a profitable project and a money-losing one. Agencies that systematize progressive enhancement engineering deliver more consistently, scope more accurately, and retain clients longer than agencies that handle it ad-hoc.

## The Framework

The Performance Budget: LCP < 2.5s, FID < 100ms, CLS < 0.1. Images: WebP with AVIF fallback, lazy-loaded below fold. Fonts: preloaded, font-display: swap, subset to used characters. JavaScript: code-split by route, tree-shaken, no bundle > 200KB. CSS: no unused styles in production.

## How to Apply This

**Step 1: Assess the current state.** Before changing anything, document how progressive enhancement engineering is currently handled. What works? What breaks? Where does the team spend unnecessary time? The assessment reveals whether the problem is process, people, tools, or all three.

**Step 2: Define the standard.** What does good look like for progressive enhancement engineering in this agency, for this project type, at this scale? The standard should be specific enough that two team members would independently produce similar outputs. If the standard is vague, the results will be inconsistent.
...
**Step 4: Measure and improve.** Track whether the system is working. The key metrics for progressive enhancement engineering: Lighthouse scores (target: 90+ per category), TypeScript coverage, test coverage (target: 60%+ for critical paths), build time, bundle size, deployment frequency, post-deployment error rate. Review quarterly. Adjust based on what the data shows, not what feels right.

## Real-World Use Cases

**Early-stage agency (1-5 people):** At this scale, progressive enhancement engineering is often handled by the founder directly. The priority is documenting what works before the team grows. Create a simple version — a one-page checklist or template — that captures the founder's judgment so it can be delegated.

**Growth-stage agency (5-20 people):** At this scale, progressive enhancement engineering must be delegated and systematized. The founder can't be in every loop. Build the standard, train the team, and create quality checks that catch problems without creating bottlenecks. The goal is consistent output without the founder's involvement in every instance.

**Scaled agency (20+ people):** At this scale, progressive enhancement engineering needs governance — who owns the standard, how it's updated, how new team members are trained, and how exceptions are handled. The system should be self-maintaining: documented enough that a new PM or lead can manage it without institutional knowledge from the founders.

## Common Mistakes

**Custom solutions for solved problems — building auth from scratch instead of using proven libraries.** This is the most common failure mode and the one that costs the most in rework, client friction, and margin erosion.

**No performance budget — shipping sites that score 40 on Lighthouse.** This usually happens when the agency is moving too fast to follow the process — which is exactly when the process matters most.

**No TypeScript — losing type safety that catches bugs before production.** This compounds over time. Each instance is small. The cumulative effect is significant.

## The Agency-Specific Dimension

Agency work has constraints that product teams and freelancers don't face. Multiple clients means progressive enhancement engineering must work across different brands, industries, and stakeholder dynamics. Parallel projects mean the team handles progressive enhancement engineering for 3-5 clients simultaneously. Fixed budgets mean there's no room for the process to be inefficient. External deadlines mean the timeline is real. These constraints make systematizing progressive enhancement engineering not optional but essential.
...
When progressive enhancement engineering is the current bottleneck in delivery or operations. When quality in this area is inconsistent across projects or team members. When the agency is scaling and needs progressive enhancement engineering to work without the founder in the loop. When post-project retrospectives identify this as a recurring issue.
